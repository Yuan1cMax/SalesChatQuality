...
> 前司需要在较短时间内验证方案可行性，因此首版优先交付最小可运行 MVP，并以 `1` 条真实会话完成端到端回归。该结果用于验证采集、解析、计时和评价链路能够闭环，不代表已经完成批量覆盖或长期准确率评测。公开仓库已脱敏，不包含后台域名、账号、客户数据或真实截图。
...
- 当前版本定位为已完成实际场景验证的最小 MVP，不将单条回归外推为批量覆盖或生产级准确率。
